...
func _ready() -> void:
	...
	if FileAccess.file_exists(path):
		...
		var parsed: Variant = JSON.parse_string(txt)  # <- TIPADO
...
func r1(x: float) -> float: return snappedf(x, 0.1)   # 1 decimal
func r3(x: float) -> float: return snappedf(x, 0.001)  # 3 decimales
...
func calc(zone_packet: Dictionary, crop_name: String) -> Dictionary:
	if not CROPS.has(crop_name):
		return {"error": "Cultivo no definido"}

	var c: Dictionary = CROPS[crop_name]
